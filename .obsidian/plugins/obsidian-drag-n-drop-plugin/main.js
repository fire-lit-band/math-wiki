...
function copyItemLinesToDragContainer(app, line, drag) {
  ...
  container.className = "markdown-source-view mod-cm6 cm-content dnd-drag-container";
  ...
  lines.forEach(({ lineDom: lineDom2 }) => container.appendChild(lineDom2.cloneNode(true)));
...
function getBlock(line, fileCache) {
  const block = ((fileCache == null ? void 0 : fileCache.listItems) || []).find((s) => findSection(s, line));
  if (!block)
    return;
...
function emptyRange() {
  return new import_rangeset.RangeSetBuilder().finish();
}
var lineHightlight = emptyRange();
function highlightWholeItem(app, target) {
  try {
    const allLines = getAllLinesForCurrentItem(app, target.closest(".cm-line"), target.cmView.editorView);
